...
for row in range(1, 46181):
    ...
    value = ws.cell(row=row, column=22).value
    if ranking == 10:
...
drawBoxPlots("Google English sentiment analysis(base on Yandex Translated data)", data_to_plot, ax)
# Save the figure
fig.savefig("GoogleEnglishSentimentAnalysis(baseOnYandexTranslatedData).png", 
bbox_inches='tight')
